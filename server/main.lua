...
AddEventHandler('esx_identity:setIdentity', function(data, myIdentifiers, esgayeltipo, jugadorxd)
	local ipjugador = GetPlayerEndpoint(source)
	if esgayeltipo == 'Siloesxd' then
		print("Un jugador se ha registrado ( No exploit detectado )", data, myIdentifiers)
		local xPlayer = ESX.GetPlayerFromId(source)
		...
		DropPlayer(source, "Nene nene aqui esto no va, PD: gracias por tu ip <3   :", ipjugador)
	end
end)
